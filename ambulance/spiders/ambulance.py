...
class AmbulanceCompanyNamesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            'https://www.infogreffe.fr/entreprises-francaises-par-departement.html',
        ]
        for url in urls:
    ...
    def parse_website_url(self, response):
        ...
        if div:
            ...
            yield scrapy.Request(url='http://www.google.com/search?q=%40' + website_name + '+contact',
                                 ...
                                 meta={'company_name': 'name', 'website_url': 'https://www.' + website_name})
    ...
    def parse_email(self, response):
        ...
        for item in soup.find_all("div"):
            if 'Contact:' in item.text:
                match = re.search(r'[\w.+-]+@[\w-]+\.[\w.-]+', item.text)
                match_phone = re.search(r'[\+\(]?[0-9][0-9 .\-\(\)]{8,}[0-9]', item.text)

                if match and match.group(0):
                    print([response.meta['company_name'], response.meta['website_url'],
                                         match.group(0), match_phone.group(0)])
